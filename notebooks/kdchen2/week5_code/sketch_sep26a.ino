...
//color and lux based adjustment will be additive/subtractive

//LED driver takes input (0...255) = 255*(x/4096) for x in (0...4096)
float adjust(float target_output, float sensor_val, float curr_output, float tolerance){
  float output_inc = 0;
  float increment = target_output*tolerance;
  float adj_increment = sensor_val*tolerance;
  if ((sensor_val < target_output-increment) && (curr_output += adj_increment < 4096)){
...
//each pwm will be driven by something like this
//led_out0 = led_out0 + adjust(...) + adjust(...);
//each adjustment is based on lux or temperature
//the cool LED's temperature adjust will probably need (4095-target_temp) for target_output
